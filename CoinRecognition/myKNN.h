...
public:
	...
	myKNN(int VS = 0, float tD[15][100] = { 0 }, int l[15] = { 0 }, float pD[10][100] = { 0 }, int pl[15] = { 0 });

	bool train_mlp_classifier(const string& filename_to_save);
	void predict_display_results(const Ptr<KNearest>& model, const string& filename_to_save);
	float getVal(int c);
